...
const getAgreement=async (req,res)=>{
    res.send({
        ...
        message: '获取成功！',
        data: {
            summary:['1.你可以查看完整版《用户协议》和《隐私政策》以便了解我们收集、使用、共享、存储信息的情况，以及对信息的保护措施。','2.在浏览使用时，我们会收集、使用设备标识信息用于推荐。','3.我们可能会基于IP地址对应的城市向你展示、推荐你所在地区的本地相关信息或内容，城市位置无需使用位置权限，仅通过IP地址确定城市及相关信息，不会收集精确位置信息。','4.我们可能会申请相册(存储)权限，以实现设置、更换头像、完成应用升级、实现相关图片和小说的下载功能。','5我们可能会由请由话权限，以保障软件服务的安全运营及效率、完成一告和信息的推送和统计、并实现在App内直接拨打电话的功能。请你放心，我们不会通过该权限获取你的电话号码、通话内容，也不会在你不知情的情况下拨打电话。','6.为实现信息分享、参加相关活动、综合统计分析等目的所必需，我们可能会调用剪切板并使用与功能相关的最小必要信息(口令、链接、统计参数等)。','如果你同意请点击下面的按钮以接受我们的服务。']
        }
    })
}
module.exports = {
